  ...
  console.log(triggeredSensors)
	...
	if (triggered == false) {
      ...
      io.emit('sensorData', triggeredSensors)
		  triggered = true
		} else if (sensorZero == 107) {
		  // 1 + 3
		  triggeredSensors = [0,2]
			io.emit('sensorData', triggeredSensors)
		  triggered = true
		} else if (sensorZero == 59) {
		  // 1 + 4
		  triggeredSensors = [0,3]
			io.emit('sensorData', triggeredSensors)
		  triggered = true
		} else if (sensorZero == 123 && sensorOne == 251) {
		  // 1 + 5
		  triggeredSensors = [0,4]
			io.emit('sensorData', triggeredSensors)
		  triggered = true
		} else if (sensorZero == 123 && sensorOne == 254) {
		  // 1 + 6
		  triggeredSensors = [0,5]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 123 && sensorOne == 239) {
		  // 1 + 7
		  triggeredSensors = [0,6]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 123 && sensorOne == 191) {
		  // 1 + 8
		  triggeredSensors = [0,7]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 123 && sensorTwo == 251) {
		  // 1 + 9
		  triggeredSensors = [0,8]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 123 && sensorTwo == 254) {
		  // 1 + 10
		  triggeredSensors = [0,9]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 123 && sensorTwo == 239) {
		  // 1 + 11
		  triggeredSensors = [0,10]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 123 && sensorTwo == 191) {
		  // 1 + 12
		  triggeredSensors = [0,11]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 123 && sensorThree == 251) {
		  // 1 + 13
		  triggeredSensors = [0,12]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 123 && sensorThree == 254) {
		  // 1 + 14
		  triggeredSensors = [0,13]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 123 && sensorThree == 239) {
		  // 1 + 15
		  triggeredSensors = [0,14]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 123 && sensorThree == 191) {
		  // 1 + 16
		...
		if (sensorZero == 110) {
		  ...
		  triggeredSensors = [1,2]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 62) {
		  // 2 + 4
		  triggeredSensors = [1,3]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 126 && sensorOne == 251) {
		  // 2 + 5
		  triggeredSensors = [1,4]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 126 && sensorOne == 254) {
		  // 2 + 6
		  triggeredSensors = [1,5]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 126 && sensorOne == 239) {
		  // 2 + 7
		  triggeredSensors = [1,6]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 126 && sensorOne == 191) {
		  // 2 + 8
		  triggeredSensors = [1,7]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 126 && sensorTwo == 251) {
		  // 2 + 9
		  triggeredSensors = [1,8]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 126 && sensorTwo == 254) {
		  // 2 + 10
		  triggeredSensors = [1,9]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 126 && sensorTwo == 239) {
		  // 2 + 11
		  triggeredSensors = [1,10]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 126 && sensorTwo == 191) {
		  // 2 + 12
		  triggeredSensors = [1,11]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 126 && sensorThree == 251) {
		  // 2 + 13
		  triggeredSensors = [1,12]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 126 && sensorThree == 254) {
		  // 2 + 14
		  triggeredSensors = [1,13]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 126 && sensorThree == 239) {
		  // 2 + 15
		  triggeredSensors = [1,14]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 126 && sensorThree == 191) {
		  // 2 + 16
		...
		if (sensorZero == 47) {
		  ...
		  triggeredSensors = [2,3]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 111 && sensorOne == 251) {
		  // 3 + 5
		  triggeredSensors = [2,4]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 111 && sensorOne == 254) {
		  // 3 + 6
		  triggeredSensors = [2,5]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 111 && sensorOne == 239) {
		  // 3 + 7
		  triggeredSensors = [2,6]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 111 && sensorOne == 191) {
		  // 3 + 8
		  triggeredSensors = [2,7]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 111 && sensorTwo == 251) {
		  // 3 + 9
		  triggeredSensors = [2,8]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 111 && sensorTwo == 254) {
		  // 3 + 10
		  triggeredSensors = [2,9]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 111 && sensorTwo == 239) {
		  // 3 + 11
		  triggeredSensors = [2,10]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 111 && sensorTwo == 191) {
		  // 3 + 12
		  triggeredSensors = [2,11]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 111 && sensorThree == 251) {
		  // 3 + 13
		  triggeredSensors = [2,12]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 111 && sensorThree == 254) {
		  // 3 + 14
		  ...
		  triggeredSensors = [2,14]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 111 && sensorThree == 191) {
		  // 3 + 16
		...
		// Fourth sensor
		if (sensorZero == 63 && sensorOne == 251) {
		  // 4 + 5
		  triggeredSensors = [3,4]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 63 && sensorOne == 254) {
		  // 4 + 6
		  triggeredSensors = [3,5]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 63 && sensorOne == 239) {
		  // 4 + 7
		  triggeredSensors = [3,6]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 63 && sensorOne == 191) {
		  // 4 + 8
		  triggeredSensors = [3,7]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 63 && sensorTwo == 251) {
		  // 4 + 9
		  triggeredSensors = [3,8]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 63 && sensorTwo == 254) {
		  // 4 + 10
		  triggeredSensors = [3,9]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 63 && sensorTwo == 239) {
		  // 4 + 11
		  triggeredSensors = [3,10]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 63 && sensorTwo == 191) {
		  // 4 + 12
		  triggeredSensors = [3,11]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 63 && sensorThree == 251) {
		  // 4 + 13
		  triggeredSensors = [3,12]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 63 && sensorThree == 254) {
		  // 4 + 14
		  triggeredSensors = [3,13]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 63 && sensorThree == 239) {
		  // 4 + 15
		  triggeredSensors = [3,14]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorZero == 63 && sensorThree == 191) {
		  // 4 + 16
		...
		//8th sensor
		if (sensorOne == 191 && sensorTwo == 251) {
		  // 8 + 9
		  triggeredSensors = [7,8]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorOne == 191 && sensorTwo == 254) {
		  // 8 + 10
		...
		// 12th sensor
		if (sensorTwo == 191 && sensorThree == 251) {
		  // 12 + 13
		  triggeredSensors = [11,12]
			io.emit('sensorData', triggeredSensors)
			triggered = true
		} else if (sensorTwo == 191 && sensorThree == 254) {
		  // 12 + 14
	...
	if (triggered == true && sensorZero == 127 && sensorOne == 255 && sensorTwo == 255 && sensorThree == 255) {
